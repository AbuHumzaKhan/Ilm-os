...
const datasets: Dataset[] = [
  {
    ...
    rows: [["P101", "Laptop Stand", "Accessories", "North", 18, 21600], ["P102", "Wireless Mouse", "Accessories", "West", 42, 33600], ["P103", "Keyboard", "Accessories", "South", 27, 40500], ["P104", "Monitor 24", "Displays", "East", 15, 52500], ["P105", "USB-C Hub", "Accessories", "North", 31, 37200], ["P106", "Webcam", "Accessories", "West", 23, 34500], ["P107", "Desk Lamp", "Office", "South", 19, 17100], ["P108", "Headset", "Accessories", "East", 34, 40800], ["P109", "SSD 1TB", "Storage", "North", 12, 62400], ["P110", "Docking Station", "Accessories", "West", 9, 49500]],
  },
  {
    ...
    rows: [["P101", "Jan", "North", 24, 28800, "Closed"], ["P104", "Jan", "East", 15, 52500, "Closed"], ["P108", "Feb", "East", 34, 40800, "Closed"], ["P103", "Feb", "South", 27, 40500, "Closed"], ["P105", "Mar", "North", 31, 37200, "Open"], ["P110", "Mar", "West", 9, 49500, "Open"], ["P102", "Apr", "West", 42, 33600, "Closed"], ["P109", "Apr", "North", 12, 62400, "Closed"], ["P106", "May", "West", 23, 34500, "Open"], ["P107", "May", "South", 19, 17100, "Closed"]],
  },
  {
    id: "missing-records",
    name: "Product Master",
    ...
    rows: [["C201", "Laptop", "Electronics", 899, 15, "Active"], ["C202", "Mouse", "Electronics", 25, 100, "Active"], ["C203", "Keyboard", "Electronics", 45, 80, "Active"], ["C204", "Monitor", "Electronics", 199, 40, "Active"], ["C205", "Printer", "Accessories", 120, 30, "Active"], ["C206", "Desk Chair", "Furniture", 150, 25, "Active"], ["C207", "Desk", "Furniture", 300, 20, "Active"], ["C208", "Notebook", "Stationery", 5, 200, "Active"], ["C209", "Pen", "Stationery", 2, 500, "Active"], ["C210", "Whiteboard", "Accessories", 90, 15, "Active"]],
  },
  {
    id: "modernization",
    ...
    rows: [["E101", "Aarav", "Sales", "North", 100000, 112500], ["E102", "Meera", "Sales", "West", 90000, 87500], ["E103", "Kabir", "Sales", "South", 110000, 121000], ["E104", "Sara", "Sales", "East", 95000, 101500], ["E105", "Zoya", "Operations", "North", 75000, 79000], ["E106", "Rohan", "Operations", "West", 82000, 80000], ["E107", "Ishita", "Finance", "South", 68000, 72000], ["E108", "Arman", "Finance", "East", 70000, 66500], ["E109", "Naina", "Sales", "North", 105000, 119000], ["E110", "Rehan", "Sales", "West", 98000, 103000]],
  },
];
  ...
  return (
    <LearningModeShell active="business" eyebrow="" title="" description="" variant="workspace">
      <section className="scenario-hero">
        ...
        <div className="scenario-switcher" aria-label="Choose scenario"><button type="button" onClick={() => chooseScenario((selected + scenarios.length - 1) % scenarios.length)} aria-label="Previous scenario">‹</button><strong>{scenarioNumber}/04</strong><button type="button" onClick={() => chooseScenario((selected + 1) % scenarios.length)} aria-label="Next scenario">›</button></div>
      </section>

      <section className="workspace-grid">
        <div className="workspace-left">
          <section className="panel dataset-panel">
            <div className="panel-head"><div className="panel-title"><span className="panel-icon blue">▦</span><div><strong>Dataset: {dataset.name}</strong><small>Editable · Excel mode</small></div></div><div className="panel-actions"><button type="button" onClick={() => setResetKey((value) => value + 1)}>↻ Reset data</button><button type="button">＋ Add rows</button></div></div>
            <BusinessDatasetSheet dataset={dataset} resetKey={resetKey} />
          </section>

          <section className="panel work-panel">
            <div className="panel-head compact-head"><div className="panel-title"><span className="panel-icon purple">▤</span><div><strong>Your work area</strong><small>Write your lookup formula here</small></div></div><span className="cell-help">A–F columns · 1–30 rows</span></div>
            <div className="mini-sheet"><div className="corner" /><div className="col-head">A</div><div className="col-head">B</div><div className="col-head">C</div><div className="col-head">D</div>{workIds.map((id, index) => <div className="sheet-row" key={`${id}-${index}`}><span className="row-head">{index + 2}</span><span className="cell key-cell">{id}</span><span className="cell">{hasRun ? resultRows[index]?.value ?? "" : ""}</span><span className="cell">{hasRun && resultRows[index]?.found ? "Matched" : ""}</span><span className={`cell ${hasRun && !resultRows[index]?.found ? "error-cell" : ""}`}>{hasRun && !resultRows[index]?.found ? "Product not found" : ""}</span></div>)}</div>
          </section>
        </div>

        <aside className="workspace-right">
          <section className="panel instructions-panel"><div className="panel-title"><span className="panel-icon cyan">✓</span><div><strong>Task instructions</strong><small>Follow these steps</small></div></div><ol>{scenario.taskSteps.map((step, index) => <li key={step}><span>{index + 1}</span><p>{step}</p></li>)}</ol></section>

          <section className="panel formula-panel"><div className="panel-head compact-head"><div className="panel-title"><span className="panel-icon blue">ƒx</span><div><strong>Formula</strong><small>Enter your spreadsheet logic</small></div></div><button type="button" className="text-button" onClick={() => setFormula(scenario.datasetId === "missing-records" ? '=IFERROR(VLOOKUP(A2,A2:F11,2,FALSE),"Product not found")' : `=VLOOKUP(A2,A2:F11,${scenario.lookupColumn},FALSE)`)}>Formula guide</button></div><label htmlFor="formula">Write your formula in B2</label><div className="formula-input"><input id="formula" value={formula} onChange={(event) => { setFormula(event.target.value); setHasRun(false); }} spellCheck={false} /><span>↗</span></div><div className="formula-actions"><button type="button" className="run-button" onClick={() => setHasRun(true)} disabled={!formula.trim()}>▶ Run formula</button><button type="button" onClick={() => { setFormula(""); setHasRun(false); }}>↻ Clear</button><button type="button" onClick={() => setHasRun(true)} disabled={!formulaLooksValid}>Auto fill (B2:B{workIds.length + 1})</button></div></section>

          <section className="panel result-panel"><div className="panel-head compact-head"><div className="panel-title"><span className="panel-icon green">✓</span><div><strong>Result</strong><small>{hasRun ? "Formula evaluated" : "Run your formula to check"}</small></div></div>{hasRun && <span className="success-badge">Success</span>}</div><div className="result-tabs"><button className="active" type="button">View result</button><button type="button">Expected output</button></div><div className="result-table"><div className="result-row result-head"><span>Row</span><span>Customer ID</span><span>Product Name</span><span>Message</span></div>{resultRows.map((row, index) => <div className="result-row" key={`${row.lookupId}-${index}`}><span>{index + 2}</span><span>{row.lookupId}</span><span>{hasRun && row.found ? row.value : "—"}</span><span>{hasRun && !row.found ? <b>{row.message}</b> : hasRun ? "" : "Run formula"}</span></div>)}</div>{hasRun && !formulaLooksValid && <div className="result-warning">The formula ran, but it does not look like an exact-match VLOOKUP yet. Check the lookup range and final FALSE argument.</div>}</section>
        </aside>
      </section>

      <section className="learning-note"><span>Learning flow</span><strong>Scenario → dataset → formula → result</strong><p>No unrelated recommendations interrupt the exercise. The same workspace can later receive server-side evaluation and saved mastery data.</p></section>
